        ...
        <x-table.body.row :stripe="$loop->even" :border="!$loop->last">
            <x-table.body.cell>
                <div class="flex items-center whitespace-nowrap dark:text-white">
                    ...
                    <div class="pl-3">
                        <div class="text-base font-semibold" data-tooltip-target="tooltip-user-{{ $user->id }}">{{ $user->display_name }}</div>

                        <div id="tooltip-user-{{ $user->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-600 text-center">
                            {{ $user->full_name }}
