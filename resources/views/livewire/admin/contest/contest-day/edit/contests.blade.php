<div>
    <x-table.container
        ...
        @foreach($contests as $contest)
            <x-table.content.row :with-stripe="$loop->even" :with-border="!$loop->last">
                <x-table.content.column>{{ $contest->name }}</x-table.content.column>
                <x-table.content.column>{{ $contest->start_time->format('H:i') }}</x-table.content.column>
                <x-table.content.column>{{ $contest->end_time->format('H:i') }}</x-table.content.column>
    ...
    <div class="flex justify-end mt-3">
        <x-form.button
            name="Neuen Contest erstellen"
            modal="create"
            :full-width="false" />
    </div>

    <x-crud.confirm-delete
        what="{{  $deleteContest?->name }}"
        id="contest"
        wire="deleteContest" />
</div>
