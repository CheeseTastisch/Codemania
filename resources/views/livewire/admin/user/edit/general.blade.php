<x-form.x type="container">
    ...
    <div class="flex flex-col gap-2">
        @if($user->email_verified_at == null)
            <x-button.big.livewire id="verify-email" action="verifyEmail" loading full-width>
                E-Mail bestätigen
            </x-button.big.livewire>
        @endif

        <x-button.big.livewire id="send-password-reset" action="sendPasswordReset" loading full-width>
            Passwort zurücksetzen
        </x-button.big.livewire>

        @if($user->hasCompleted2Fa())
            <x-button.big.livewire id="reset-2fa" action="reset2Fa" loading full-width>
