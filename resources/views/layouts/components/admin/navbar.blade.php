    ...
    <div class="container flex flex-wrap items-center justify-between mx-auto">
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
            <ul class="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-slate-200 dark:bg-gray-800 md:dark:bg-slate-800 dark:border-gray-700">
                <li>
                    <a href="{{ route('admin.dashboard') }}" class="nav-link hover:text-accent-400 dark:hover:text-accent-600 @if(Route::is('admin.dashboard')) !text-accent-400 dark:!text-accent-600 @endif">
                        Dashboard
                    </a>
                </li>
                <li>
                    <a href="{{ route('admin.faq') }}" class="nav-link hover:text-accent-400 dark:hover:text-accent-600 @if(str(Route::currentRouteName())->startsWith('admin.faq')) !text-accent-400 dark:!text-accent-600 @endif">
                        FAQ
                    </a>
                </li>
                <li>
                    <a href="#" class="nav-link hover:text-accent-400 dark:hover:text-accent-600 @if(str(Route::currentRouteName())->startsWith('admin.contest')) !text-accent-400 dark:!text-accent-600 @endif">
                        Contests
                    </a>
                </li>
                <li>
                    <a href="#" class="nav-link hover:text-accent-400 dark:hover:text-accent-600 @if(str(Route::currentRouteName())->startsWith('admin.user')) !text-accent-400 dark:!text-accent-600 @endif">
                        Benutzer
                    </a>
                </li>
            </ul>
