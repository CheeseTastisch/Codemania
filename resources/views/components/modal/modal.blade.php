<div id="{{ $id }}" wire:ignore.self tabindex="-1" :aria-hidden="!open" role="dialog" aria-modal="true"
     x-data="{ open: false }" x-on:modal-open="open = true" x-on:moda-close="open = false" x-on:modal-toggle="open = !open" @keydown.escape.window="open = false"
    ...
    <div class="relative w-full {{ $getMaxWidth }} max-h-full">
        <div class="relative bg-slate-200 dark:bg-slate-800 rounded-lg shadow-lg z-[60]" @click.outside="open = false">
            <!-- Modal header -->
            @if($title)
    ...
    <div class="bg-gray-900 dark:bg-opacity-80 fixed inset-0 z-50"></div>
</div>
